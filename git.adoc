...
文件的 SHA 值及类型信息。
+
* `9f4d96d` 和 `2e5de8b` 分别代表 hello.txt 文件在工作目录下和暂存区中的 SHA 值。Git 按照文件内容为文件生成一个唯一的 SHA 值，这些 SHA 值与文件名无关，因此文件改动前后的 SHA 值是不同的。关于更多 Git SHA 信息，请参考 <branch>。
...
将这些信息概括起来就是：在工作目录中，hello.txt 文件中第一行内容被修改成了 `Hello git diff`，同时又向该文件追加了两行内容 `Line1` 和 `Line2`。
