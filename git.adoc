...
== git log: 查看历史提交记录
每次提交代码后，Git 都会为我们自动生成一条提交记录，通过查看提交记录，我们可以很方便的追踪代码的历史修改记录。

Git 子命令 `log` 用于查看所有的历史提交记录，并按照提交 commit 时的时间顺序由近到远打印出来。
...
在这个例子中，我们没有为 log 指定任何参数，所有日志信息按照默认格式全部打印出来。

TIP: 如果历史记录过多导致当前屏幕无法将日志全部展示出来时，Git 则以交互模式展示历史记录。在交互模式下，`j` 用于向下滚动一行内容，`k` 用于向上滚动一行内容，`q` 用于退出交互模式。

=== git log 的一些高级用法
Git log 命令为我们提供了大量的可选参数，通过这些参数，我们可以对历史记录进行格式化输出、过滤输出结果、搜索特定提交记录，查看某次提交的改动内容等功能。
...
==== 格式化输出历史记录
首先让我们看一下，如果使用不同的格式展示提交历史记录。采用不同的格式输出，不仅仅是输出格式的不同，输出的内容项也会有所不同。
...
关于更多格式信息，请参考 https://git-scm.com/docs/pretty-formats[pretty formats]。
